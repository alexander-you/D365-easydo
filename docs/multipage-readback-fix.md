...
### למה זה עבד עד עכשיו?
הזרימה נבנתה ונבדקה מול **תבניות בדיקה של עמוד אחד** (למשל `68729`, `68793`). בתבניות אלה כל השדות נמצאו ב‑`payload.data[0]`, ולכן `first(...)` כלל את כולם. ההנחה הסמויה "העמוד הראשון מכיל את כל השדות" התבררה כשגויה כאשר התקבל חוזה אמיתי רב‑עמודים (`69985` — "חוזה הרשאה 2026", 26 עמודים).
...
| Throttling ב‑Dataverse ביצירת רשומות | תלוי סביבה | `concurrency.repetitions = 1` (כבר מוגדר) שומר על ריצה סדרתית ומפחית את הסיכון לחריגה ממגבלות הקצב של Dataverse |
