...
> חתימה דיגיטלית מול easydo. הוא כולל את טבלאות החתימה הרגילות, טבלת יומן מסוג Elastic,
> טבלאות ה**מעטפה** הרב‑מסמכית, רשימות בחירה גלובליות, קשרים, טפסים ותצוגות — הכול עם
...
> מראש (Prefill) ולקליטת ערכים לאחר החתימה (Read Back). `alex_fieldname` תואם לשם הטכני של השדה
