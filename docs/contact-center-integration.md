...
זהו **מודל זמן אמת**, שבו ה"ערוץ" הוא **השיחה החיה עצמה**, וההפצה מתבצעת לפי **כללי
ההפצה העצמית** (easydo אינה שולחת הודעה).

### 2. ישות השיחה — `msdyn_ocliveworkitem`

השיחה החיה היא טבלת Dataverse **`msdyn_ocliveworkitem`**. היא מחזיקה את ערוץ השיחה ואת הלקוח
המקושר.

**אומת בפועל (22-06-2026)** בסביבת `demo-contact-center-en`:
...
`msdyn_ocliveworkitem` היא **ישות חריגה**: השיחה זמנית ונסגרת בסיום הטיפול, והיא טבלת
מערכת שמוסתרת מרשימת הטבלאות. לכן **אין לשייך אליה את המסמך או את בקשת החתימה**. במעמד השליחה הנציג
