...
Power Platform components to integrate with the EasyDoc signing service. No Azure
...
└───────────────┬─────────────────────────────────────────────────────┘
                ...
                ▼
        EasyDoc API — https://api.easydo.co.il/api
...
| Custom Connector | Typed EasyDoc operations (no raw HTTP actions) | ✅ |
...
2. Flow creates the EasyDoc form (`draft:true`), applies the template/fields, sets
...
4. On confirmation, flow `PUT …/send` with `meta_data.client` carrying the Contact id
   + table name.
5. A `Signature Request` row is created/updated in Dataverse with the EasyDoc form id
   and `fill_url`.

### Status polling & document return

1. A scheduled flow reads open `Signature Request` rows and calls EasyDoc *Get
   Status*.
...
The EasyDoc `meta_data.client` bag stores `{ "d365_table": "contact",
"d365_id": "<guid>", "signature_request_id": "<guid>" }`, allowing both polling and a
future webhook to map an EasyDoc form back to the originating Dynamics 365 record.

## Future evolution

- Replace polling with the EasyDoc **webhook** (`Form Submitted`) once a secure
