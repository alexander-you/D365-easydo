...
New-DVGlobalChoice -Name "alex_signaturestatus" `
    ...
    -Options @(
        @{ En="Draft";          He="טיוטה";        DescEn="The request has been created in Dynamics but not yet sent to EasyDoc."; DescHe="הבקשה נוצרה ב-Dynamics אך טרם נשלחה ל-EasyDoc." }
        ...
        @{ En="Sent";            He="נשלח";          DescEn="The request was successfully sent to the recipient through EasyDoc."; DescHe="הבקשה נשלחה בהצלחה לנמען דרך EasyDoc." }
        @{ En="Delivered";       He="נמסר";          DescEn="EasyDoc confirmed the signature request reached the recipient."; DescHe="EasyDoc אישר שבקשת החתימה הגיעה לנמען." }
...
# 2) Recipient signing status (mirrors EasyDoc assignee states)
...
New-DVGlobalChoice -Name "alex_logdirection" `
    ...
    -DescEn "Whether an integration event represents an outbound call to EasyDoc or an inbound update from EasyDoc." `
    -DescHe "האם אירוע האינטגרציה מייצג קריאה יוצאת ל-EasyDoc או עדכון נכנס מ-EasyDoc." `
    -Options @(
        @{ En="Outbound"; He="יוצא"; DescEn="A request sent from Dynamics to EasyDoc."; DescHe="בקשה שנשלחה מ-Dynamics ל-EasyDoc." }
        @{ En="Inbound";  He="נכנס"; DescEn="A status update or callback received from EasyDoc."; DescHe="עדכון סטטוס או קריאה חוזרת שהתקבלו מ-EasyDoc." }
    )
