<#
  ...
  individual EasyDoc form field. The send flow reads the Prefill rows and builds
  the prefill_data array sent to EasyDoc; Read Back rows capture what the
  recipient entered.
...
New-DVTable -Schema "alex_SignatureFieldValue" `
    ...
    -DescEn "A concrete value for a single EasyDoc form field on a specific signature request, used to prefill the field before sending or to record what the recipient entered." `
    -DescHe "ערך מוחשי לשדה טופס בודד ב-EasyDoc עבור בקשת חתימה מסוימת, המשמש למילוי מקדים של השדה לפני השליחה או לתיעוד מה שהנמען הזין." `
    -PrimaryName $pn

# ----- 3) Columns ----------------------------------------------------------
$t = "alex_signaturefieldvalue"
Write-Output "== $t =="
Add-DVColumn $t (New-DVString -Schema "alex_FieldName" -En "EasyDoc Field Name" -He "שם שדה EasyDoc" -MaxLength 100 -Required "ApplicationRequired" `
    -DescEn "Technical name of the EasyDoc form field this value targets (matches the EasyDoc Field Id on the template field mapping, e.g. custom_field_6a32cedc7ede2)." `
    -DescHe "השם הטכני של שדה הטופס ב-EasyDoc שאליו מכוון הערך (תואם למזהה שדה EasyDoc במיפוי שדות התבנית, לדוגמה custom_field_6a32cedc7ede2).")
