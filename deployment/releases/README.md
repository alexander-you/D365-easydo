...
| --- | --- | --- |
| [2.0.0.12](2.0.0.12/) | 2026-09-17 | **Fix** — *Read Signature Results* reliability. Signed-PDF download and `completedon` are no longer gated on easydo `has_data`, so **approval-only** documents get their PDF attached; crash-safe read-back filter (boolean/mask `export` no longer throws `InvalidTemplate`); an idempotency guard prevents **duplicate** read-back rows on re-poll; the envelope path now **attaches the PDF before** stamping `completedon`; and a **retry-cap** flips a persistently-failing request (dead easydo form / unretrievable PDF) to **Failed** after 48 attempts (~4 h) with `alex_errorcode`/`alex_errormessage`, surfacing it in *Requests Needing Attention* instead of retrying forever. Applies to **both** single-document and envelope paths. Flow-only change |
| [2.0.0.11](2.0.0.11/) | 2026-08-31 | **Feature** — manual signature-request cancellation with a reason. Red *"ביטול בקשת החתימה"* button in the document pane opens a confirmation dialog with a multi-line reason text area; sets `alex_status` = Cancelled (triggering the 2.0.0.9 easydo close-form flow) and stores the reason in the new `alex_cancelreason` memo. Optional global *"חייב סיבת ביטול"* toggle (`alex_requirecancelreason`) makes the reason mandatory. Single documents only (envelopes excluded) |
